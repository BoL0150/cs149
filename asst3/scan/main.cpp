...
void usage(const char *progname) {
  ...
  printf("  -m  --test <TYPE>      Run specified function on input.  Valid "
         "tests are: scan, find_repeats (default: scan)\n");
  printf("  -i  --input <NAME>     Run test on given input type. Valid inputs "
...
double cpu_exclusive_scan(int *start, int *end, int *output) {
...
int main(int argc, char **argv) {
  ...
  double studentCudaTime = 50000.;
  double thrustCudaTime = 50000.;
  ...
  if (test.compare("scan") == 0) { // test exclusive scan
    ...
    for (int i = 0; i < 3; i++) {
      thrustCudaTime = std::min(
          thrustCudaTime, cudaScanThrust(inarray, inarray + N, resultarray));
      studentCudaTime = std::min(studentCudaTime,
                                 cudaScan(inarray, inarray + N, resultarray));
      cpuTime = std::min(cpuTime,
                         cpu_exclusive_scan(inarray, inarray + N, checkarray));
    }

    // run CPU implementation to check correctness

    printf("Thrust GPU time: %.3f ms\n", 1000.f * thrustCudaTime);
    printf("Student GPU time: %.3f ms\n", 1000.f * studentCudaTime);
    ...
    int cu_size;
    for (int i = 0; i < 3; i++) {
      studentCudaTime = std::min(
          studentCudaTime, cudaFindRepeats(inarray, N, resultarray, &cu_size));
    }
