...
void saxpyCuda(int N, float alpha, float *xarray, float *yarray,
  ...
  double computationDuration = endTime2 - startTime2;
  printf("Effective BW by CUDA saxpy: overall %.3f ms\t\t, computation %.3f ms "
         "\t\t[%.3f GB/s]\n",
         1000.f * overallDuration, 1000.f * computationDuration,
